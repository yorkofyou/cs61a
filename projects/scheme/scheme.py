...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        args = rest.map(lambda lst: scheme_eval(lst, env))
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ret = self.fn(*python_args)
        except TypeError as error:
            raise SchemeError('the number of arguments is wrong') from error
        return ret
        # END PROBLEM 3
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        if names == nil:
            ...
            prev_name.rest = Pair(binding.first, nil)
            prev_name = prev_name.rest
        val = scheme_eval(binding.rest.first, env)
        if values == nil:
            values = Pair(val, nil)
            prev_values = values
        else:
            prev_values.rest = Pair(val, nil)
            prev_values = prev_values.rest
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 20
    "*** YOUR CODE HERE ***"
    validate_form(expressions, 2, 2) # Checks that expressions is a list of length at exactly 2
    target = expressions.first
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a dynamic-scoped call evaluated in the evaluated environment."""
