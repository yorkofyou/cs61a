...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        if feral_hogs and abs(num_of_dice - s0) == 2:
            score0 += 3
        s0 = take_turn(num_of_dice, score1, dice)
        score0 += s0
        if not is_swap(score0, score1):
...
def swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if is_swap(score + s, opponent_score):
        if score + s <= opponent_score:
            return 0
    elif s >= cutoff:
        return 0
